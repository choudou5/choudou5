package com.choudou5.base.util.ip;
...
import com.choudou5.base.util.ip.model.TaoBaoIpEntry;
 ...
 * @Author：xuhaowen
 * @Date：2018-06-02
 * @Ref: https://gitee.com/mjtop/JTopCMSV3/blob/master/src/cn/com/mjsoft/framework/util/IPSeeker.java
 */
public class IPParseUtil {

    private static Logger log = LoggerFactory.getLogger(IPParseUtil.class);
...
//                + File.separator + "WEB-INF" + File.separator + "config" + File.separator + "ip.dat" );
    ...
    public List getIPEntriesDebug( String s )
    {
        ...
        for ( long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH )
        {
            ...
            if( temp != -1 )
            {
                ...
                {
                    IPEntry entry = new IPEntry(loc.country, loc.getArea());
                    // 得到起始IP
                    readIP( offset - 4, b4 );
                    entry.setBeginIp(getIpStringFromBytes(b4));
                    // 得到结束IP
                    readIP(temp, b4);
                    entry.setEndIp(getIpStringFromBytes(b4));
                    // 添加该记录
                    ret.add( entry );
                }
            }
    ...
    public String getAddress( String ip ) {
        ...
        return address.trim().replaceAll( "CZ88.NET", "" ).replaceAll( "对方和您在同一内部网", "" );
    }
    ...
    private static byte[] getIpByteArrayFromString( String ip ) {
        ...
        {
            log.info( "从ip的字符串形式得到字节数组形式报错" + e );
        }
        return ret;
    }
     ...
     * @return
     */
    public static TaoBaoIpEntry getInfoByTaoBaoApi(String ip) {
        if(StrUtil.isBlank(ip))
            return null;
        String resp = null;
        try {
            resp = HttpUtil.get(API + ip, 2000);
        } catch (Exception e) {
            log.error("淘宝接口解析ip失败", e);
            return null;
        }
        JSONObject jsonObject = JSON.parseObject(resp);
        return jsonObject.getObject("data", TaoBaoIpEntry.class);
    }
